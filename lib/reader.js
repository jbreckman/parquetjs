...
const PathStreamer = require('./pathStreamer');
...
class ParquetEnvelopeReader {
  ...
  search2(paths, options) {
    ...
    let pathReader = new PathReader(reader, paths, options.timer);
  ...
  readFlatPage(offsetIndex, pageNumber, records) {
    // TODO: faster than assign?
    let column = Object.assign({},offsetIndex.column);
    column.metadata = Object.assign({},column.metadata);
    column.meta_data.data_page_offset = offsetIndex.page_locations[pageNumber].offset;
    ...
    return this.readColumnChunk(this.schema, column).then(chunk => {
      chunk.column = column;
      return parquet_shredder.materializeValues(this.schema, column.meta_data.path_in_schema.join(','), chunk);
    });
  }
