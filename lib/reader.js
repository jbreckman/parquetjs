...
class ParquetEnvelopeReader {
  ...
  search(paths, options) {
    ...
    async function pages(rowGroup) {
      ...
      let offsetsPromise = pathReader.loadOffsetInfo(rowGroup);

      let excludedPages = await pathReader.findIgnorePages(rowGroup);

      // wait for offset indices to finish
      let offsets = await offsetsPromise;

      // push forward any pages that haven't been excluded
      for (let i = 0; i < offsets.pageCount; i++) {
        if (!excludedPages.has(i)) {
          this.push({offsets, pageNo: i, rowGroupNo: rowGroup.no});
        }
      }
    }

    async function results(page) {
      let records = await pathReader.getRecords(page.offsets, page.pageNo);
      records.forEach(rec => {
        this.push(rec)
      });
      return;
