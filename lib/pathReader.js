...
class MultiReader {
  ...
  async getRecords(offsetInfo, pageNo, rowGroupNo) {
    ...
    records = await this.loadFilteredFields(offsetInfo, pageNo, records, cache, rowGroupNo);
    ...
    await this.loadRecords(offsetInfo, pageNo, records, cache, rowGroupNo);

    return filteredResults || records;
  }

  async loadRecords(offsetInfo, pageNo, records, cache, rowGroupNo) {
    await Promise.all(this.items.map(item => item.loadRecords(offsetInfo, pageNo, records, cache, rowGroupNo)))
    return records;
  }

  async loadFilteredFields(offsetInfo, pageNo, records, cache, rowGroupNo) {
    await Promise.all(this.items.map(item => item.loadFilteredFields(offsetInfo, pageNo, records, cache, rowGroupNo)))
    return records;
  }
}
...
class PathReaderItem {
  ...
  async _loadRecords(offsetInfo, pageNo, records, cache, rowGroupNo) {
    let columnRecords = await (cache[this.columnIndex] || (cache[this.columnIndex] = memoize(`${this.reader.id}_page_${rowGroupNo}_${pageNo}_${this.source}`, 
        async () => {
  ...
  async loadFilteredFields(offsetInfo, pageNo, records, cache, rowGroupNo) {
    ...
    return await this._loadRecords(offsetInfo, pageNo, records, cache, rowGroupNo);
  }
