...
class MultiReader {
  ...
  getRecordCount(rowGroup) {
    ...
    return memoize(`${reader.id}_columnIndex${rowGroup.no}_${columnIndex}`, () => reader.readColumnIndex(rowGroup.columns[columnIndex]))
...
class PathReaderItem {
  ...
  loadOffsetInfo(rowGroup, target) {  
    ...
    return memoize(`${this.reader.id}_offsetIndex${rowGroup.no}_${columnIndex}`, () => this.reader.readOffsetIndex(column))
      .then(offsetIndex => {
