...
class PathReaderItem {
  ...
  _loadRecords(offsetInfo, pageNo, records, rowGroupNo) {
    ...
    return memoize(`${reader.id}_page_${rowGroupNo}_${pageNo}_${source}`, () => reader.readPage(offsetInfo[columnIndex], pageNo, []).then(r => {
        if (source) {
          r = r.map(d => JSON.parse(d[path]));
        }
        else {
          r = r.map(d => d[path]);
        }
        ...
        if (source) {
          if (records.length === 0) {
            for (var i = 0; i < columnRecords.length; i++) {
              ...
              let newRecord = {};
              for (var key in columnRecord) {
                newRecord[key] = columnRecord[key];
              }
